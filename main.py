...
prompt before GPT optimization = "
Act as a system. Range this reviews by rave with rate on a scale from 1 to 10, where 10 is the most enthusiastic review,
1 is the most negative review. Return me only a list of rate numbers separate with ','"
...
def fun_GPT_respons(review_list: list):
    prompt = f'Rank reviews on a scale of 1 to 10. Give only the numeric ratings separated by commas. ' \
             f'Reviews: {review_list}'

    # try:
    response = openai.Completion.create(engine=GPT_ENGINE, prompt=prompt)
    # except ConnectionError:
    #     logging.error(f'{ConnectionError}')
    logging.debug(f'{response}')

    return [int(i) for i in response['choices'][0]['text'].split(',')]
